...
function Marcas() {
    ...
    //     let filtroMarcas = produtos.filter(p => {
    //         return p.id !== produto.id
    //     })
    //     setMarcas(filtroMarcas);
    //     localStorage.setItem("@produtos", JSON.stringify(filtroMarcas));
    //
                    ...
                    <table width={'100%'}>
                        ...
                        {marcas.toSorted((a,b) => a.name.localeCompare(b.name)).map(marca =>
                            <tr key={marca.id}>
                                ...
                                <td width={'1%'}><button onClick={()=>editMarca(marca)}>Alterar</button></td>
                            </tr>
                        )}
                        </tbody>
                    </table>
